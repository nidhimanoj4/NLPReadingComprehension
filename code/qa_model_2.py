...
class Encoder(object):
    def __init__(self, size, vocab_dim, max_passage_length):
        self.size = size
    ...
    def encode(self, inputs, sequence_lengths, masks=None, encoder_state_input=None):
        ...
        with vs.variable_scope("Co-Attention-Summaries"):
            ...
            # A^Q = softmax(C)
            # A^P = softmax(C^T)
            q_concat_outputs = tf.transpose(a=q_concat_outputs, perm=(0, 2, 1)) # [batch_size, 2 * hidden_size, max_question_length]
            transposed_p_concat_outputs = p_concat_outputs
            p_concat_outputs = tf.transpose(a=p_concat_outputs, perm=(0, 2, 1)) # [batch_size, 2 * hidden_size, max_passage_length]
            ...
            alt_coattention = tf.transpose(a=tf.nn.softmax(logits=raw_coattention, dim=0), perm=(0, 2, 1)) # [batch_size, max_question_length, max_passage_length]
        ...
        print(u_concat_outputs.get_shape())
        return u_concat_outputs # [batch_size, max_passage_length, 2 * hidden_size]
...
class Decoder(object):
    ...
    # We return (batch_size, passage_length, 2)
    def create_fc_layer(self, inputs, max_len_passage, name_space):
        ...
        outputs = tf.reshape(outputs, [-1, max_len_passage, 2])
        return outputs
    ...
    def decode(self, knowledge_rep):
        ...
        # We return tuple: ( (batch_size, max_passage_length, 2), (batch_size, max_passage_length, 2) )
...
class QASystem(object):
    ...
    def __init__(self, encoder, decoder, rev_vocab, args):
        ...
        self.learning_rate = args.learning_rate # The learning rate we want to use
        ...
        self.print_val_every_batch = args.print_val_every_batch # When to calculate stuff on validation
        ...
        self.a_t_placeholder = tf.placeholder(tf.int32, shape=(None, None))
        self.e_t_placeholder = tf.placeholder(tf.int32, shape=(None, None)) # Same format as above but for end index
        ...
        self.num_saved = 0 # Counts how many times we saved so we can update checkpoint file
    ...
    def setup_loss(self, preds):
        ...
        with vs.variable_scope("loss"):
            ...
            # Add in loss
            cross_a_t = tf.boolean_mask(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=preds[0], labels=self.a_t_placeholder), self.mask_passage_placeholder)
            cross_e_t = tf.boolean_mask(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=preds[1], labels=self.e_t_placeholder), self.mask_passage_placeholder)
            loss = tf.reduce_mean(cross_a_t) + tf.reduce_mean(cross_e_t)
            return loss
    ...
    def optimize(self, session, train_x, train_y):
        ...
        input_feed = {
            ...
            self.passage_sequence_lengths : len(passages_batch[0])*np.ones(len(passages_batch)),
            self.question_sequence_lengths: len(questions_batch[0])*np.ones(len(questions_batch)),
    ...
    def answer(self, session, test_x, raw_predictions_batch=None):
        ...
        predictions_a_t = tf.nn.softmax(raw_predictions_a_t).eval() # Perform softmax over each word
        predictions_e_t = tf.nn.softmax(raw_predictions_e_t).eval() # Perform softmax over each word
        ...
        for i in xrange(len(predictions_a_t)):
            ...
            start_index = np.argmax(prediction_a_t[0:end_of_passage, 1]) # Find the word with the largest prob of being start index
            if start_index == end_of_passage - 1: # Start index is the last word
                ...
                end_index = np.argmax(prediction_e_t[start_index:end_of_passage, 1]) + start_index
    ...
    def evaluate_answer(self, session, dataset, rev_vocab = None, raw_predictions_batch = None, sample=3, log=False):
        ...
        sample_indices = np.random.choice(len(dataset), sample, replace=False)
        ...
        for i in range(len(cleaned_predictions_batch)):
            ...
            true_answer_indices = []
            start_index = np.where(sample_a_t == 1)[0][0] # Which index is the true start index
            end_index =  np.where(sample_e_t == 1)[0][0] # Which index is the true end index
    ...
    def run_epoch(self, session, train_dataset, val_dataset, train_dir):
        ...
        for batch_idx in range(0, len(passages), batch_size):
            ...
            if batches_completed % self.print_val_every_batch == 0:
        ...
        train_f1, train_em = self.evaluate_answer(session, train_dataset, \
    ...
    def expand_answers_for_a_t_e_t(self, session, dataset):
        ...
        for i in xrange(len(passages)):
            a_t_answers[i, answers[i][0]] = 1
            e_t_answers[i, answers[i][1]] = 1
        del answers # Delete to conserve memory
        # Put into tuple
    ...
    def sample_validation(self, session, val_dataset, val_frac, eval_num_samples):
        ...
        validation_loss, validation_preds = self.test(session, random_val_dataset)
    ...
    def train(self, session, train_dataset, val_dataset, train_dir):
        ...
        for epoch in range(self.epochs):
            ...
            self.run_epoch(session, train_dataset, val_dataset, train_dir)
            # TODO: add in early stopping
            logging.info("Saving model")
            self.saver.save(session, self.train_dir + '/' + self.saved_name + '_' + str(self.num_saved))
            self.num_saved += 1
