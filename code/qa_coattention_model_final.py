...
class Encoder(object):
    def __init__(self, size, vocab_dim, quadratic_form=False):
        self.size = size
    ...
    def encode(self, inputs, sequence_lengths, keep_prob, masks=None, encoder_state_input=None):
        ...
        # Generate bi-lstm for passage and question using same cells
        with vs.variable_scope("Encoding-Layer"):
            # Create forward direction cell
            with vs.variable_scope('forward'):
                s_lstm_fw_cell = tf.nn.rnn_cell.LSTMCell(self.size, forget_bias=1.0, state_is_tuple=True)
            # Create backward direction cell
            with vs.variable_scope('backward'):
                s_lstm_bw_cell = tf.nn.rnn_cell.LSTMCell(self.size, forget_bias=1.0, state_is_tuple=True)

            # Create bilateral LSTM for passage
            p_outputs, p_output_states = tf.nn.bidirectional_dynamic_rnn(cell_fw=s_lstm_fw_cell, cell_bw = s_lstm_bw_cell, \
                inputs = passages,  dtype=tf.float64, scope="Shared-Bi-LSTM", sequence_length=passage_sequence_lengths)
            ...
            # Create bilateral LSTM for question
            q_outputs, q_output_states = tf.nn.bidirectional_dynamic_rnn(cell_fw=s_lstm_fw_cell, cell_bw = s_lstm_bw_cell, \
                inputs = questions,  dtype=tf.float64, scope="Shared-Bi-LSTM", sequence_length=question_sequence_lengths)

            # Perform same concatenations as above
        ...
        # Allow for variation between question and document encoding space
        with vs.variable_scope("Variation-Layer"):
            q_concat_outputs_w_dropout = tf.contrib.layers.fully_connected(
                inputs=q_concat_outputs_w_dropout,
                num_outputs=(2 * self.size),
                activation_fn=tf.tanh,
                weights_initializer=tf.contrib.layers.xavier_initializer(),
                biases_initializer=tf.constant_initializer(0.0),
            )
            q_concat_outputs_w_dropout = tf.nn.dropout(q_concat_outputs_w_dropout, keep_prob)
        ...
        with vs.variable_scope("Co-Attention-Summaries"):
            ...
            if self.quadratic_form is True:
                p_concat_outputs_w_dropout = tf.contrib.layers.fully_connected(
                    inputs=p_concat_outputs_w_dropout,
                    num_outputs=(2 * self.size),
                    activation_fn=None,
                    weights_initializer=tf.contrib.layers.xavier_initializer(),
...
class Decoder(object):
    ...
    def create_fc_layer(self, inputs, max_len_passage, output_size, name_space):
        ...
        outputs = tf.reshape(outputs, [-1, max_len_passage, output_size])
        return outputs

    def decode(self, knowledge_rep, keep_prob):
        """
        ...
        # We first compute a two layers of fully connected to get start index distribution. We use ReLu in between
        # We then concatenate this final start distribution with the knowledge rep and run two other layers of FC with ReLu
        # in between. This is to get the end index representation
        ...
        # Create first layer for start indices
        a_t_outputs = self.create_fc_layer(knowledge_rep, max_len_passage, 100, "start_index_prediction_layer_1")
        # Compute ReLu
        a_t_outputs = tf.nn.relu(a_t_outputs)
        # Add dropout
        a_t_outputs_dropout = tf.nn.dropout(a_t_outputs, keep_prob)
        # Compute second layer for start indices
        a_t_outputs = self.create_fc_layer(a_t_outputs_dropout, max_len_passage, 1, "start_index_prediction_layer_2")

        # Now concatenate each of these output vectors with knowledge rep
        end_concat = tf.concat(2, [knowledge_rep, a_t_outputs])

        # Create first layer
        e_t_outputs = self.create_fc_layer(end_concat, max_len_passage, 100, "end_index_prediction_layer_1")
        # Compute ReLu
        e_t_outputs = tf.nn.relu(e_t_outputs)
        # Add dropout
        e_t_outputs_dropout = tf.nn.dropout(e_t_outputs, keep_prob)
        # Create second layer for end indices
        e_t_outputs = self.create_fc_layer(e_t_outputs_dropout, max_len_passage, 1, "end_index_prediction_layer_2")

        # Reduce to get reduce of the extra 1 dimension
        a_t_outputs = tf.reshape(a_t_outputs, [-1, max_len_passage])
        e_t_outputs = tf.reshape(e_t_outputs, [-1, max_len_passage])
...
class QASystem(object):
    ...
    def __init__(self, encoder, decoder, rev_vocab, args):
        ...
        self.should_use_new_loss = args.should_use_new_loss
        self.should_use_dp_prediction = args.should_use_dp_prediction
        ...
        if self.should_use_new_loss:
            self.a_t_placeholder = tf.placeholder(tf.float64, shape=(None, None))
            self.e_t_placeholder = tf.placeholder(tf.float64, shape=(None, None))
        else:
            self.a_t_placeholder = tf.placeholder(tf.int32, [None])
            self.e_t_placeholder = tf.placeholder(tf.int32, [None]) # Same format as above but for end index
    ...
    def setup_predictions(self):
        ...
        decode_output = self.decoder.decode(encode_output, self.keep_prob)
    ...
    def setup_loss(self, preds):
        ...
        with vs.variable_scope("loss"):
            ...
            # Add in loss
            if self.should_use_new_loss:
                #cross_a_t = tf.nn.l2_loss(preds[0] - self.a_t_placeholder)
                #cross_e_t = tf.nn.l2_loss(preds[1] - self.e_t_placeholder)
                #cross_a_t = tf.nn.softmax_cross_entropy_with_logits(logits=tf.boolean_mask(preds[0], self.mask_passage_placeholder), labels=tf.boolean_mask(self.a_t_placeholder, self.mask_passage_placeholder))
                cross_a_t = tf.nn.softmax_cross_entropy_with_logits(logits=preds[0], labels=self.a_t_placeholder)
                cross_e_t = tf.nn.softmax_cross_entropy_with_logits(logits=preds[1], labels=self.e_t_placeholder)
            else:
    ...
    def setup_embeddings(self):
        ...
        with vs.variable_scope("embeddings"):
            embs = tf.Variable(self.pretrained_embeddings, trainable=False)
    ...
    def get_dp_answer_prediction(self, session, prediction_a_t, prediction_e_t, passage_end):
        # First we construct m_e_t. m_e_t[i] = max(e_t[i:m]) where m is the length of e_t
        # Use passage end to protect against predicted a padded answer
        m_e_t = np.zeros(passage_end)
        m_e_t[-1] = prediction_e_t[passage_end-1]
        i = passage_end - 2
        while i >= 0:
            m_e_t[i] = max(prediction_e_t[i], m_e_t[i+1])
            i -= 1

        # Now we get the max index for a_t that gives the maximum sum for start and end

        start_index = np.argmax(prediction_a_t[0:passage_end]+m_e_t)
        # We know the best end index is the one with max value after the start index
        if start_index == passage_end - 1: # Start index is the last word
            end_index = start_index
        else:
            # Set end word as the word after the start index with largest probability of being end index
            end_index = np.argmax(prediction_e_t[start_index:passage_end]) + start_index
    ...
    def answer(self, session, test_x, raw_predictions_batch=None):
        ...
        for i in xrange(num_predictions):
            ...
            if self.should_use_dp_prediction:
                start_index, end_index = self.get_dp_answer_prediction(session, prediction_a_t, prediction_e_t, end_of_passage)
            else:
    ...
    def evaluate_answer(self, session, dataset, rev_vocab = None, raw_predictions_batch = None, sample=3, log=False):
        ...
        cleaned_predictions_batch = self.answer(session, \
            test_x=(sample_contexts, sample_questions, (sample_a_ts, sample_e_ts), sample_context_masks, sample_question_masks), \
        ...
        for i in range(len(cleaned_predictions_batch)):
            ...
            true_answer_indices = []
            if self.should_use_new_loss:
                start_index = np.where(sample_a_t >= .9)[0][0]
                end_index = np.where(sample_e_t >= .9)[0][0]
            else:
    ...
    def expand_answers_for_a_t_e_t(self, session, dataset):
        ...
        if self.should_use_new_loss:
            a_t_answers = np.zeros(shape=(passages.shape))
            e_t_answers = np.zeros(shape=(passages.shape))
        else:
            a_t_answers = np.zeros(shape=(passages.shape[0]))
            e_t_answers = np.zeros(shape=(passages.shape[0]))
        for i in xrange(len(passages)):
            if self.should_use_new_loss:
                # Set center distributions
                a_t_answers[i, answers[i][0]] = .95
                e_t_answers[i, answers[i][1]] = .95
                # Add left window for a_t
                if answers[i][0] >= 2:
                    a_t_answers[i, answers[i][0] - 2] = .01 / 2.0
                    a_t_answers[i, answers[i][0] - 1] = .04 / 2.0
                elif answers[i][0] == 1:
                    a_t_answers[i, answers[i][0] - 1] = .05 / 2.0
                else:
                    a_t_answers[i, answers[i][0]] += .05 / 2.0
                # Add right window for a_t
                if answers[i][0] <= len(a_t_answers[i]) - 3:
                    a_t_answers[i, answers[i][0] + 2] = .01 / 2.0
                    a_t_answers[i, answers[i][0] + 1] = .04 / 2.0
                elif answers[i][0] == len(a_t_answers[i]) - 2:
                    a_t_answers[i, answers[i][0] + 1] = .05 / 2.0
                else:
                    a_t_answers[i, answers[i][0]] += .05 / 2.0
                # Add right windoe for e_t
                if  answers[i][1] <= len(e_t_answers[i]) - 3:
                    e_t_answers[i, answers[i][1]+1] = .04 / 2.0
                    e_t_answers[i, answers[i][1]+2] = .01 / 2.0
                elif answers[i][1] == len(e_t_answers[i]) - 2:
                    e_t_answers[i, answers[i][1]+1] = .05 / 2.0
                else:
                    e_t_answers[i, answers[i][1]] += .05 / 2.0
                # Add left window for e_t
                if answers[i][1] >= 2:
                    e_t_answers[i, answers[i][1] - 2] = .01 / 2.0
                    e_t_answers[i, answers[i][1] - 1] = .04 / 2.0
                elif answers[i][1] == 1:
